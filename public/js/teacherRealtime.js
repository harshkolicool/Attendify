document.addEventListener("DOMContentLoaded", function init() {
    ...
    socket.on("attendance:pending-review", function(payload) {
        addPendingReview(payload);
        showTeacherToast((payload.studentName || "A student") + " requested a manual attendance review.", "warning");
    });

    function addPendingReview(payload) {
        const list = document.getElementById("pendingReviewList");
        const countPill = document.getElementById("pendingReviewCount");
        if (!list) return;

        // Remove empty state if present
        const emptyState = list.querySelector(".suspicious-attempt-empty");
        if (emptyState) emptyState.remove();

        const li = document.createElement("li");
        li.className = "suspicious-attempt-item pending-review-item";
        li.dataset.studentId = payload.studentId;
        li.dataset.sessionId = payload.sessionId;

        const top = document.createElement("div");
        top.className = "suspicious-attempt-top";
        const studentBox = document.createElement("div");
        const name = document.createElement("strong");
        name.textContent = payload.studentName;
        const enrollment = document.createElement("small");
        enrollment.textContent = payload.enrollmentNumber;
        studentBox.appendChild(name);
        studentBox.appendChild(enrollment);
        ...
        const reason = document.createElement("p");
        reason.textContent = "GPS weak/missing. Student is waiting for your manual approval.";

        const meta = document.createElement("div");
        meta.className = "suspicious-meta";
        if (payload.gpsAccuracy) {
            meta.appendChild(createMetaSpan("fa-solid fa-crosshairs", "Accuracy: " + Math.round(payload.gpsAccuracy) + "m"));
        }
        if (payload.confidenceScore) {
            meta.appendChild(createMetaSpan("fa-solid fa-percent", "Confidence: " + Math.round(payload.confidenceScore) + "%"));
        }
        ...
        btnApprove.className = "teacher-primary-btn";
        btnApprove.style.padding = "4px 12px";
        btnApprove.style.fontSize = "12px";
        btnApprove.innerHTML = '<i class="fa-solid fa-check"></i> Approve';
        btnApprove.onclick = function() { reviewAction(payload.sessionId, payload.studentId, "approve", li); };

        const btnReject = document.createElement("button");
        btnReject.className = "teacher-secondary-btn";
        btnReject.style.padding = "4px 12px";
        btnReject.style.fontSize = "12px";
        btnReject.style.color = "#dc2626";
        btnReject.style.borderColor = "#fca5a5";
        btnReject.innerHTML = '<i class="fa-solid fa-xmark"></i> Reject';
        btnReject.onclick = function() { reviewAction(payload.sessionId, payload.studentId, "reject", li); };

        const btnRetry = document.createElement("button");
        btnRetry.className = "teacher-secondary-btn";
        btnRetry.style.padding = "4px 12px";
        btnRetry.style.fontSize = "12px";
        btnRetry.innerHTML = '<i class="fa-solid fa-rotate-right"></i> Ask to Retry';
        btnRetry.onclick = function() { reviewAction(payload.sessionId, payload.studentId, "ask-retry", li); };
        ...
        const count = list.querySelectorAll("li.pending-review-item").length;
        if (countPill) countPill.textContent = count + " pending";
    }

    function reviewAction(sessionId, studentId, action, liElement) {
        fetch("/teacher/attendance/" + sessionId + "/review/" + studentId + "/" + action, {
            method: "POST",
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                if (count === 0) {
                    list.innerHTML = '<li class="suspicious-attempt-empty">No pending reviews at this time.</li>';
                }
            } else {
                showTeacherToast(data.message || "Action failed", "danger");
            }
        })
        .catch(err => {
            console.error(err);
            showTeacherToast("Network error. Try again.", "danger");
        });
    }
